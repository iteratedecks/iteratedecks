...
uses
  ...
  cxTreeView, cxTL, cxInplaceContainer, cxButtonEdit, EDImports;
          ...
          begin
            ...
            try
              ...
              begin
                ...
                cbOwnedCardsFilter.Enabled := true;
                if cbOwnedCardsFilter.Enabled then
                  LoadOwnedCards(sLocalDir + 'wildcard\ownedcards.txt');
              end;
              cardList.Clear;
              for I := 1 to 20-1 do
...
begin
  ...
  begin
    if (cbOwnedCardsFilter.ItemIndex > 0) and
       (((cbOwnedCardsFilter.ItemIndex = 1) AND (CheckOwnedCard(Cards[i].Id) = 0)) OR
        ((cbOwnedCardsFilter.ItemIndex = 2) AND (CheckOwnedCard(Cards[i].Id) > 0))) then
    begin
      Images[i].Hide;
      continue;
    end;
...
begin
  ...
  cbOwnedCardsFilter.Enabled := cbCheckOnlyCardsOwned.Enabled;
